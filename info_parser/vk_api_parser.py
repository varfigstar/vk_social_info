...
def get_group_info_from_api(group_id: str) -> dict:
    ...
    resp = requests.get(url, params={"group_id": group_id}).json()
...
class Parser:
    ...
    def ack_task(self):
        counter = self.redis_cli.get("TASKS_COUNTER")
        self.redis_cli.set("TASKS_COUNTER", counter-1)
    ...
    async def _get_data_from_db(self, group_id: str) -> dict:
        ...
        except VkGroupModel.ObjectDoesNotExist:
            return
        if data:
    ...
    async def get_group_info(self, group_id: str) -> dict:
        data = await self._get_data_from_redis(group_id)
        if not data:
